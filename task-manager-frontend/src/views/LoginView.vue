      ...
      <div class="col-md-6 col-lg-5">
        <div class="card shadow-lg border-0 rounded-lg">
          <div class="card-header bg-primary text-white rounded-top py-3">
            <h3 class="text-center mb-0"><i class="bi bi-person-fill me-2"></i> Login</h3>
          </div>
          <div class="card-body p-4 p-md-5">
            <form @submit.prevent="handleLogin">
              ...
              <button type="submit" class="btn btn-primary w-100 py-2" :disabled="isSubmitting">
                <i class="bi bi-box-arrow-in-right me-2" v-if="!isSubmitting"></i>
                <span v-if="isSubmitting" class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
                {{ isSubmitting ? 'Logging In...' : 'Login' }}
              </button>
              <p class="mt-4 text-center text-muted">
                Don't have an account? <router-link to="/register" class="text-decoration-none">Sign Up</router-link>
              </p>
            </form>
          </div>
        </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    isSubmitting.value = false; // Set submitting state back to false
  }
};
</script>
